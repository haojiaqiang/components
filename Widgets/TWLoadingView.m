...
- (void)loadSubviews {
    ...
    self.direction = TWPopoverViewDirectionFromCenter;
    self.closeDirection = TWPopoverViewDirectionFromCenter;
    self.maskColor = kRGBA(0, 0, 0, .05);
    
    self.closable = NO;
    
    CGRect r = CGRectMake(0, 0, 110, 110);
    _imageView = [[TWImageView alloc] initWithFrame:r];
    [self addSubview:_imageView];
    
    __weak id weakSelf = self;
    self.popviewDidShow = ^(TWPopoverView *popoverView) {
...
- (void) startAnimations {
    if (!_backView) {
        ...
        _backView = [[TWView alloc] initWithFrame:r andRadius:r.size.width/2];

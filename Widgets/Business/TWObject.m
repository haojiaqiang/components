...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary {
    ...
    NSError *error;
    TWObject *object = [self initWithDictionary:dictionary error:&error];
    if (error) {
        DLog(@"%@", error);
    }
    return object;
...
- (instancetype)initWithJSON:(NSString *)jsonString {
    ...
    NSError *error;
    TWObject *object = [self initWithString:jsonString error:&error];
    if (error) {
        DLog(@"%@", error);
    }
    return object;
...
@implementation TWObject (JSONModel)

...
- (id)stringToModel:(Class)model {
    ...
    if (!data) {
        DLog(@"%@ of stringToModel failed", NSStringFromClass(self.class));
        return nil;
    }
    ...
    if (error || !dict) {
        DLog(@"%@ of stringToModel error: %@", NSStringFromClass(self.class), error);
        return nil;
    }
...
+ (NSString*)modelToString:(HTJSONModel *)model {
    NSError *error;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:[model toDictionary] options:0 error:&error];
    if (!jsonData || error) {
        DLog(@"%@ of modelToString failed", NSStringFromClass(self.class));
        return nil;
    }
...
- (int)textLineNumWithFont:(UIFont *)font constrainedToWidth:(CGFloat)width {
    CGFloat realHeight = [self returnSize:font MaxWidth:width].height;
    CGFloat oneLineHeight = font.lineHeight;
    return (int)ceilf(realHeight/oneLineHeight);
}

- (CGSize)textSizeForOneLineWithFont:(UIFont *)font {
    return [self returnSize:font MaxWidth:MAXFLOAT];
}
